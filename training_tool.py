...
class _DashHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if url.path == "/index":
            ...
            runs = []
            if base.is_dir():
                # iterate train* subfolders and add mtime + file list
        ...
        if url.path == "/ui":
            # try to locate dashboard.html next to the app
            cand = [
...
def start_dashboard_server(base_dir: str | Path) -> tuple[str, socketserver.TCPServer]:
    """Start a background HTTP server exposing /index and /file for the dashboard."""
...
def require_yaml(parent: QtWidgets.QWidget | None = None):
    ...
    If not installed, ask the user to install it, try to install via pip, then import again.
    Raises ImportError if still missing.
    """
    try:
...
class StepIndicator(QtWidgets.QWidget):
    ...
    def _update_style(self):
        if self.is_active:
            ...
            self.circle.setStyleSheet("""
                QLabel#stepCircle { background: #10b981; color: white;
                    border-radius: 16px; font-weight: 600; }""")
            self.title_label.setStyleSheet("color: #94a3b8;")
        else:
            self.circle.setStyleSheet("""
                QLabel#stepCircle { background: #334155; color: #64748b;
                    border-radius: 16px; font-weight: 600; }""")
...
class StepModel(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        title.setStyleSheet("background: transparent; font-size: 26px; color: #27b6f3; font-weight: 700;")
        ...
        def key_for(label_text: str) -> str:
            return {"YOLO Segmentation": "seg", "YOLO Detection": "det", "Detectron Segmentation": "dseg", "Detectron Detection": "ddet"}[label_text]
        ...
        add_card(1, 0, "Detectron Segmentation")
        add_card(1, 1, "Detectron Detection")
    ...
    def value(self) -> str:
        for _, rb, name in self._cards:
            if rb.isChecked():
                ...
                if name == "Detectron Segmentation": return "dseg"
                if name == "Detectron Detection": return "ddet"
        return "seg"
...
class StepTrain(QtWidgets.QWidget):
    ...
    def __init__(self, mode: str = "seg"):
        ...
        # Build Detectron pages
        self.detectron_page = self._build_page_detectron()
        self.detectron_det_page = self._build_page_detectron_det()
    ...
    def _jump_to_mode(self, mode: str):
        ...
        # Default label for row 1
        if self.nav.item(1): self.nav.item(1).setText("Segmentation")

        if mode == "seg":
            self._swap_seg_slot(None)
            if self.nav.item(2): self.nav.item(2).setHidden(True)
            self.nav.setCurrentRow(0)

        elif mode == "det":
            self._swap_seg_slot(None)
            if self.nav.item(1): self.nav.item(1).setHidden(True)
            self.nav.setCurrentRow(0)

        elif mode == "dseg":
            if self.nav.item(1): self.nav.item(1).setText("Detectron")
            self._swap_seg_slot(self.detectron_page)
            if self.nav.item(0): self.nav.item(0).setHidden(True)
            if self.nav.item(2): self.nav.item(2).setHidden(True)
            if self.nav.item(3): self.nav.item(3).setHidden(True)
            ...
            if torch.cuda.is_available():
                ...
                if shutil.which("nvidia-smi"):
                    try:
                        ...
                        if out:
                            tip = "NVIDIA driver detected: " + ", ".join([ln.strip() for ln in out.splitlines() if ln.strip()])
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _build_page_detectron(self) -> QtWidgets.QWidget:
        ...
        title = QtWidgets.QLabel("Detectron Segmentation")
        ...
        chip_css = """
            ...
            background: rgba(148, 163, 184, 0.18);
            border: 1px solid rgba(148, 163, 184, 0.35);
            border-radius: 10px; padding: 4px 10px; min-height: 36px;
        """
        def mk_lbl(t):
            lb = QtWidgets.QLabel(t); lb.setAlignment(QtCore.Qt.AlignCenter); lb.setStyleSheet(chip_css); return lb

        # Detectron install (optional)
        self.d_repo_edit = QtWidgets.QLineEdit("detectron2_repo")
        repo_browse = QtWidgets.QToolButton(); repo_browse.setText("Browse…")
        repo_browse.clicked.connect(lambda: (lambda dn: self.d_repo_edit.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Detectron2 repo folder", os.path.expanduser("~"))
        ))
        repo_row = QtWidgets.QHBoxLayout(); repo_row.setContentsMargins(0,0,0,0); repo_row.setSpacing(8)
        repo_row.addWidget(self.d_repo_edit, 1); repo_row.addWidget(repo_browse, 0)
        repo_wrap = self._wrap(repo_row)

        self.d_install_cb = QtWidgets.QCheckBox("Clone & install Detectron2 before training")

        # Dataset (no YOLO data.yaml here)
        self.d_data_edit = QtWidgets.QLineEdit()
        data_browse = QtWidgets.QToolButton(); data_browse.setText("Browse…")
        data_browse.clicked.connect(lambda: (lambda dn: self.d_data_edit.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Data root folder", os.path.expanduser("~"))
        ))
        data_row = QtWidgets.QHBoxLayout(); data_row.setContentsMargins(0,0,0,0); data_row.setSpacing(8)
        data_row.addWidget(self.d_data_edit, 1); data_row.addWidget(data_browse, 0)
        data_wrap = self._wrap(data_row)

        self.d_train_edit = QtWidgets.QLineEdit("train10")
        self.d_test_edit  = QtWidgets.QLineEdit("test10")
        self.d_classes_edit = QtWidgets.QLineEdit("top_rectangle")

        # Model & HPs (Detectron only)
        from_models = [
            "COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml",
            "COCO-InstanceSegmentation/mask_rcnn_R_101_FPN_3x.yaml",
            "COCO-InstanceSegmentation/mask_rcnn_X_101_32x8d_FPN_3x.yaml",
        ]
        self.d_model_combo = QtWidgets.QComboBox(); self.d_model_combo.addItems(from_models)

        self.d_ims_per_batch = QtWidgets.QSpinBox(); self.d_ims_per_batch.setRange(1, 64); self.d_ims_per_batch.setValue(2)
        self.d_base_lr       = QtWidgets.QDoubleSpinBox(); self.d_base_lr.setDecimals(6); self.d_base_lr.setRange(1e-6, 1.0); self.d_base_lr.setValue(0.00025)
        self.d_max_iter      = QtWidgets.QSpinBox(); self.d_max_iter.setRange(10, 100000); self.d_max_iter.setValue(1000)
        self.d_workers       = QtWidgets.QSpinBox(); self.d_workers.setRange(0, 16); self.d_workers.setValue(2)

        self.d_device_combo  = QtWidgets.QComboBox(); self._fill_device_combo(self.d_device_combo)

        self.d_output_edit   = QtWidgets.QLineEdit("detectron_runs")
        out_browse = QtWidgets.QToolButton(); out_browse.setText("Browse…")
        out_browse.clicked.connect(lambda: (lambda dn: self.d_output_edit.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Output folder", os.path.expanduser("~"))
        ))
        out_row = QtWidgets.QHBoxLayout(); out_row.setContentsMargins(0,0,0,0); out_row.setSpacing(8)
        out_row.addWidget(self.d_output_edit, 1); out_row.addWidget(out_browse, 0)
        out_wrap = self._wrap(out_row)

        # Layout
        r = 1
        grid.addWidget(mk_lbl("Detectron2 repo"), r,0); grid.addWidget(repo_wrap, r,1,1,3); r+=1
        grid.addWidget(self.d_install_cb, r,1,1,3); r+=1

        grid.addWidget(mk_lbl("Data root"), r,0); grid.addWidget(data_wrap, r,1,1,3); r+=1
        grid.addWidget(mk_lbl("Train folder"), r,0); grid.addWidget(self.d_train_edit, r,1)
        grid.addWidget(mk_lbl("Test folder"),  r,2); grid.addWidget(self.d_test_edit,  r,3); r+=1
        grid.addWidget(mk_lbl("Classes"), r,0); grid.addWidget(self.d_classes_edit, r,1,1,3); r+=1

        grid.addWidget(mk_lbl("Model config"), r,0); grid.addWidget(self.d_model_combo, r,1,1,3); r+=1
        grid.addWidget(mk_lbl("IMS_PER_BATCH"), r,0); grid.addWidget(self.d_ims_per_batch, r,1)
        grid.addWidget(mk_lbl("BASE_LR"), r,2); grid.addWidget(self.d_base_lr, r,3); r+=1
        grid.addWidget(mk_lbl("MAX_ITER"), r,0); grid.addWidget(self.d_max_iter, r,1)
        grid.addWidget(mk_lbl("NUM_WORKERS"), r,2); grid.addWidget(self.d_workers, r,3); r+=1

        grid.addWidget(mk_lbl("Device"), r,0); grid.addWidget(self.d_device_combo, r,1)
        grid.addWidget(mk_lbl("Output dir"), r,2); grid.addWidget(out_wrap, r,3); r+=1
    ...
    def _build_page_detectron_det(self) -> QtWidgets.QWidget:
        ...
        title = QtWidgets.QLabel("Detectron Detection (Pascal VOC)")
        title.setStyleSheet("color:#e5eefb; font-weight:700;")
        grid.addWidget(title, 0, 0, 1, 4)
        ...
        # Dataset (VOC)
        self.voc_images_root = QtWidgets.QLineEdit()
        btn_images = QtWidgets.QToolButton(); btn_images.setText("Browse…")
        btn_images.clicked.connect(lambda: (lambda dn: self.voc_images_root.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "VOC Images Root (has JPEGImages & ImageSets/Main)", os.path.expanduser("~"))
        ))
        row = QtWidgets.QHBoxLayout(); row.setContentsMargins(0,0,0,0); row.setSpacing(8)
        row.addWidget(self.voc_images_root, 1); row.addWidget(btn_images, 0)
        voc_images_wrap = self._wrap(row)

        self.voc_labels_root = QtWidgets.QLineEdit()
        btn_labels = QtWidgets.QToolButton(); btn_labels.setText("Browse…")
        btn_labels.clicked.connect(lambda: (lambda dn: self.voc_labels_root.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "VOC XML Labels Root (folder with .xml)", os.path.expanduser("~"))
        ))
        row = QtWidgets.QHBoxLayout(); row.setContentsMargins(0,0,0,0); row.setSpacing(8)
        row.addWidget(self.voc_labels_root, 1); row.addWidget(btn_labels, 0)
        voc_labels_wrap = self._wrap(row)

        self.voc_split_name = QtWidgets.QLineEdit("train")
        self.voc_classes_edit = QtWidgets.QLineEdit("chip_mark,cut_piece,out_piece,curling_damage,bs,dent,dr")

        # Model config (COCO-Detection Faster R-CNN)
        det_models = [
            "COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml",
            "COCO-Detection/faster_rcnn_R_101_FPN_3x.yaml",
            "COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml",
        ]
        self.d2det_model_combo = QtWidgets.QComboBox(); self.d2det_model_combo.addItems(det_models)

        # Optional: custom start weights
        self.d2det_weights_edit = QtWidgets.QLineEdit("")
        btn_w = QtWidgets.QToolButton(); btn_w.setText("Browse…")
        btn_w.clicked.connect(lambda: (lambda fn: self.d2det_weights_edit.setText(fn) if fn else None)(
            QtWidgets.QFileDialog.getOpenFileName(self, "Select Detectron .pth (optional)", os.path.expanduser("~"), "PyTorch Weights (*.pth *.pkl);;All Files (*.*)")[0]
        ))
        rw = QtWidgets.QHBoxLayout(); rw.setContentsMargins(0,0,0,0); rw.setSpacing(8)
        rw.addWidget(self.d2det_weights_edit, 1); rw.addWidget(btn_w, 0)
        weights_wrap = self._wrap(rw)

        # HPs
        self.d2det_ims_per_batch = QtWidgets.QSpinBox(); self.d2det_ims_per_batch.setRange(1, 64); self.d2det_ims_per_batch.setValue(6)
        self.d2det_max_iter      = QtWidgets.QSpinBox(); self.d2det_max_iter.setRange(10, 100000); self.d2det_max_iter.setValue(3000)
        self.d2det_steps_edit    = QtWidgets.QLineEdit("")
        self.d2det_workers       = QtWidgets.QSpinBox(); self.d2det_workers.setRange(0, 32); self.d2det_workers.setValue(2)
        self.d2det_device_combo  = QtWidgets.QComboBox(); self._fill_device_combo(self.d2det_device_combo)

        # Output dir
        self.d2det_output_edit = QtWidgets.QLineEdit("detectron_det_runs")
        btn_out = QtWidgets.QToolButton(); btn_out.setText("Browse…")
        btn_out.clicked.connect(lambda: (lambda dn: self.d2det_output_edit.setText(dn) if dn else None)(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Output folder", os.path.expanduser("~"))
        ))
        ro = QtWidgets.QHBoxLayout(); ro.setContentsMargins(0,0,0,0); ro.setSpacing(8)
        ro.addWidget(self.d2det_output_edit, 1); ro.addWidget(btn_out, 0)
        out_wrap = self._wrap(ro)

        # Layout rows
        r = 1
        grid.addWidget(mk_lbl("Images root (JPEGImages + ImageSets/Main)"), r,0); grid.addWidget(voc_images_wrap, r,1,1,3); r+=1
        grid.addWidget(mk_lbl("Labels root (.xml folder)"), r,0); grid.addWidget(voc_labels_wrap, r,1,1,3); r+=1
        grid.addWidget(mk_lbl("Split"), r,0); grid.addWidget(self.voc_split_name, r,1)
        grid.addWidget(mk_lbl("Classes"), r,2); grid.addWidget(self.voc_classes_edit, r,3); r+=1

        grid.addWidget(mk_lbl("Model config"), r,0); grid.addWidget(self.d2det_model_combo, r,1,1,3); r+=1
        grid.addWidget(mk_lbl("Start weights (.pth, optional)"), r,0); grid.addWidget(weights_wrap, r,1,1,3); r+=1

        grid.addWidget(mk_lbl("IMS_PER_BATCH"), r,0); grid.addWidget(self.d2det_ims_per_batch, r,1)
        grid.addWidget(mk_lbl("MAX_ITER"), r,2); grid.addWidget(self.d2det_max_iter, r,3); r+=1
        grid.addWidget(mk_lbl("STEPS (comma)"), r,0); grid.addWidget(self.d2det_steps_edit, r,1)
        grid.addWidget(mk_lbl("NUM_WORKERS"), r,2); grid.addWidget(self.d2det_workers, r,3); r+=1

        grid.addWidget(mk_lbl("Device"), r,0); grid.addWidget(self.d2det_device_combo, r,1)
        grid.addWidget(mk_lbl("Output dir"), r,2); grid.addWidget(out_wrap, r,3); r+=1

        return card

    def _card(self) -> QtWidgets.QFrame:
        f = QtWidgets.QFrame(); f.setObjectName("card"); return f
    ...
    def _equalize_label_sizes(self, labels: list[QtWidgets.QLabel]):
        ...
        for l in labels:
            l.setWordWrap(False)
            l.setMinimumSize(w, h); l.setMaximumHeight(h); l.setFixedHeight(h)

    def _open_merge_dialog(self):
        dlg = DatasetMergeDialog(self)
        if dlg.exec_() == QtWidgets.QDialog.Accepted:
            if dlg.out_base_yaml: self.data_edit.setText(dlg.out_base_yaml)
    ...
    def _build_page_dataset(self):
        ...
        ver_label.setStyleSheet(chip_css); ver_label.setAlignment(QtCore.Qt.AlignCenter)
        ...
        self.data_edit.setStyleSheet("""
            QLineEdit { color:#ffffff; font-size:16px; padding: 8px 12px; }
        ...
        hl = QtWidgets.QHBoxLayout(); hl.setSpacing(8)
        hl.addWidget(self.data_edit, 1); hl.addWidget(self.btn_browse_data, 0)

        data_label = QtWidgets.QLabel("data.yaml:")
        data_label.setStyleSheet(chip_css); data_label.setAlignment(QtCore.Qt.AlignCenter)
        ...
        merge_btn.setStyleSheet("""
            QPushButton { color: white; font-weight: 700;
        ...
        lw.setContentsMargins(0, 8, 0, 0); lw.setSpacing(0)
    ...
    def _build_page_seg(self):
        ...
        rb_css = """
            QRadioButton { color:#e6edf7; font-size:15px; font-weight:600; padding:6px 10px; border-radius:8px; }
            QRadioButton::indicator { width:18px; height:18px; }
            QRadioButton:hover { background: rgba(148,163,184,0.14); }
            QRadioButton:checked { background: rgba(14,165,233,0.18); border:1px solid rgba(14,165,233,0.45); }
        """
        ...
        self.seg_browse = QtWidgets.QToolButton(); self.seg_browse.setText("Browse…")
        self.seg_browse.clicked.connect(lambda: self._browse_path(self.seg_path_edit))

        hp = QtWidgets.QHBoxLayout(); hp.setContentsMargins(0,0,0,0); hp.setSpacing(8)
        hp.addWidget(self.seg_path_edit); hp.addWidget(self.seg_browse)
        ...
        seg_wlbl = QtWidgets.QLabel("Weights path:"); seg_wlbl.setStyleSheet("color:#ffffff; font-weight:600;")
        row.addWidget(seg_wlbl); row.addLayout(hp)
    ...
    def _build_page_det(self):
        ...
        self.det_browse = QtWidgets.QToolButton(); self.det_browse.setText("Browse…")
        self.det_browse.clicked.connect(lambda: self._browse_path(self.det_path_edit))

        hp = QtWidgets.QHBoxLayout(); hp.setContentsMargins(0,0,0,0); hp.setSpacing(8)
        hp.addWidget(self.det_path_edit); hp.addWidget(self.det_browse)

        self.det_weights_container = QtWidgets.QWidget()
        row = QtWidgets.QHBoxLayout(self.det_weights_container)
        row.setContentsMargins(0,0,0,0); row.setSpacing(8)
        det_wlbl = QtWidgets.QLabel("Weights path:"); det_wlbl.setStyleSheet("color:#ffffff; font-weight:600;")
        row.addWidget(det_wlbl); row.addLayout(hp)
    ...
    def _build_page_hparams(self):
        ...
        def mk_lbl(t):
            lb = QtWidgets.QLabel(t); lb.setAlignment(QtCore.Qt.AlignCenter); lb.setStyleSheet(chip_css)
            self._hparam_labels.append(lb); return lb

        self.device_combo = QtWidgets.QComboBox(); self._populate_device_combo()
        ...
        grid.setColumnStretch(2, 1); grid.setColumnStretch(3, 2)
    ...
    def _build_page_run(self):
        ...
        self.btn_build = QtWidgets.QPushButton("Build Commands"); self.btn_build.setObjectName("navButton")
        self.btn_build.setFixedHeight(44); self.btn_build.setMinimumWidth(140)
        self.btn_build.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))

        self.btn_start = QtWidgets.QPushButton("Start Training"); self.btn_start.setObjectName("primaryNavButton")
        self.btn_start.setFixedHeight(44); self.btn_start.setMinimumWidth(160)
        ...
        btn_row.addStretch(1); btn_row.addWidget(self.btn_build); btn_row.addWidget(self.btn_start)

        v.addWidget(self.console, 1); v.addLayout(btn_row)
        self.stack.addWidget(card)
    ...
    def _apply_local_qss(self):
        self.setStyleSheet("""
            ...
            QListWidget#trainNav { background: rgba(15,23,42,0.5); border:1px solid #334155; border-radius:10px; color:#dbe7ff; }
            ...
            QFrame#card {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 rgba(30,41,59,0.55), stop:1 rgba(15,23,42,0.55));
                border:1px solid #334155; border-radius:12px;
            }
            QLabel#chip { color:#e6faff; background:#0ea5e922; border:1px solid #0ea5e9; border-radius:11px; padding:1px 10px; font-weight:700; font-size:12px; }
            QLineEdit, QComboBox { background: rgba(15,23,42,0.8); border:2px solid #334155; border-radius:8px; padding:10px 14px; color:#f1f5f9; }
            ...
            QPlainTextEdit#console {
                background:#0b1220; border:1px solid #223; border-radius:10px; color:#e5e7eb;
                font-family:Consolas,'Fira Code',monospace; font-size:12px; padding:8px;
            }
            QLineEdit, QSpinBox, QDoubleSpinBox, QComboBox, QToolButton { min-height: 10px; padding: 8px 12px; }
        """)
    ...
    def build_commands(self) -> tuple[str, list]:
        ...
        if self.mode == "dseg":
            ...
            # Optional install for Detectron2
    ...
    def _on_start(self):
        ...
        for program, args in cmds:
            self.console.appendPlainText(self._fmt_cmd(program, args))
        self.nav.setCurrentRow(4)
        # record overall start time for benchmark
        try:
    ...
    def _next_or_done(self):
        ...
        if self._idx >= len(self._cmds):
            # all done — record overall end time and show total duration
            try:
        ...
        # start the next command (no per-command benchmark logs)
        self.console.appendPlainText(f"\n[Running {self._idx+1}/{len(self._cmds)}] -> {program} " + " ".join(args))
        self._proc.start(program, args)

    def _format_td(self, td):
        """Format a timedelta into H:MM:SS.mmm or M:SS.mmm."""
        try:
...
class TrainingWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # Sidebar
        sidebar_container = QtWidgets.QWidget(); sidebar_container.setObjectName("sidebar"); sidebar_container.setFixedWidth(280)
        sidebar_layout = QtWidgets.QVBoxLayout(sidebar_container); sidebar_layout.setContentsMargins(0, 0, 0, 0); sidebar_layout.setSpacing(0)

        header = QtWidgets.QWidget(); header.setObjectName("sidebarHeader"); header.setFixedHeight(70)
        header_layout = QtWidgets.QVBoxLayout(header); header_layout.setContentsMargins(20, 16, 20, 16)
        logo = QtWidgets.QLabel("Training Wizard"); logo.setObjectName("logo")
        header_layout.addWidget(logo); header_layout.addStretch(1)

        steps_widget = QtWidgets.QWidget()
        self.steps_layout = QtWidgets.QVBoxLayout(steps_widget)
        self.steps_layout.setContentsMargins(0, 16, 0, 16); self.steps_layout.setSpacing(2)
        ...
        # Left "Dashboard" button (smaller & slightly lower)
        side_dash_wrap = QtWidgets.QWidget()
        sd_lay = QtWidgets.QHBoxLayout(side_dash_wrap)
        sd_lay.setContentsMargins(20, 24, 20, 0)  # space above to push it a bit down
        sd_lay.setSpacing(0)

        self.btn_dashboard_side = QtWidgets.QPushButton("Dashboard")
        self.btn_dashboard_side.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.btn_dashboard_side.setMinimumHeight(32)  # smaller than before
        self.btn_dashboard_side.clicked.connect(self._open_dashboard)
        self.btn_dashboard_side.setStyleSheet("""
            QPushButton {
        ...
        """)
        sd_lay.addWidget(self.btn_dashboard_side)

        self.steps_layout.addSpacing(24)  # push button a little down
        self.steps_layout.addWidget(side_dash_wrap)
        ...
        # Main area
        main_container = QtWidgets.QWidget(); main_container.setObjectName("mainContainer")
        main_layout = QtWidgets.QVBoxLayout(main_container); main_layout.setContentsMargins(0, 0, 0, 0); main_layout.setSpacing(0)

        topbar = QtWidgets.QWidget(); topbar.setObjectName("topBar"); topbar.setFixedHeight(46)
        topbar_layout = QtWidgets.QHBoxLayout(topbar); topbar_layout.setContentsMargins(12, 6, 12, 6); topbar_layout.addStretch()
        ...
        self.stack.addWidget(self.step_model); self.stack.addWidget(self.step_train)

        main_layout.addWidget(topbar); main_layout.addWidget(self.stack, 1)
        root.addWidget(main_container, 1)

        nav_bar = QtWidgets.QWidget(); nav_bar.setObjectName("navBar"); nav_bar.setFixedHeight(72)
        nav_layout = QtWidgets.QHBoxLayout(nav_bar); nav_layout.setContentsMargins(24, 12, 24, 12); nav_layout.setSpacing(10)
        self.btn_back = QtWidgets.QPushButton("← Back"); self.btn_back.setObjectName("navButton")
        self.btn_back.setFixedHeight(44); self.btn_back.setFixedWidth(110)
        self.btn_next = QtWidgets.QPushButton("Next →"); self.btn_next.setObjectName("primaryNavButton")
        self.btn_next.setFixedHeight(44); self.btn_next.setFixedWidth(110)
        self.btn_back.clicked.connect(self._go_back); self.btn_next.clicked.connect(self._go_next)
        nav_layout.addStretch(); nav_layout.addWidget(self.btn_back); nav_layout.addWidget(self.btn_next)
        main_layout.addWidget(nav_bar)

        root_frame = QtWidgets.QFrame(); root_frame.setObjectName("trainingRoot")
        wrap = QtWidgets.QVBoxLayout(root_frame); root_frame.setAutoFillBackground(True)
        root_frame.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        wrap.setContentsMargins(0, 0, 0, 0); wrap.setSpacing(0); wrap.addWidget(central)
        self.setCentralWidget(root_frame)

        self._apply_qss()
        self._update_nav()

        self._dash_httpd = None  # set when dashboard server starts

    def _open_dashboard(self):
        api_url, self._dash_httpd = start_dashboard_server(DEFAULT_RUNS_DIR)

        # Open the dashboard served by our tiny server
        url = QtCore.QUrl(f"{api_url}/ui?theme=dark")
        QtGui.QDesktopServices.openUrl(url)

        QtWidgets.QMessageBox.information(
            self, "Dashboard",
            f"Loaded runs from:\n{DEFAULT_RUNS_DIR}\n\n"
            "Tip: use the dropdown to switch runs."
        )
    ...
    def _apply_qss(self):
        self.setStyleSheet("""
            * { font-family: 'Segoe UI','SF Pro Display',-apple-system,BlinkMacSystemFont,Arial,sans-serif; font-size: 14px; }
            ...
            #trainingRoot, QWidget#mainContainer, QStackedWidget#contentStack { background: #0f1419; }
            ...
            QPushButton#navButton {
                background:#394150; color:#cbd5e1; border:1px solid #475569; border-radius:10px; font-weight:600;
            }
            ...
            QLineEdit, QComboBox { background:#0b1220; border:2px solid #334155; border-radius:8px; padding:8px 12px; color:#f1f5f9; }
            QLineEdit:focus, QComboBox:focus { border-color:#0ea5e9; }
            QPlainTextEdit#console {
                background:#0b1220; border:1px solid #223; border-radius:10px; color:#e5e7eb;
                font-family:Consolas,'Fira Code',monospace; font-size:12px; padding:8px;
