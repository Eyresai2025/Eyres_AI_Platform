    ...
    def __init__(self, text, icon_path=None, tooltip=None):
        ...
        self.setStyleSheet("""
            QPushButton {
                ...
                font-weight: 700;
                border-radius: 8px;
                margin: 4px 8px;           /* was 6px 10px */
            }
            QPushButton:hover { background-color: #6c757d; border-left: 4px solid #0d6efd; }
    ...
    def load_camera_app(self):
        try:
            ...
            try:
                ...
                QMessageBox.critical(self, "Import Error", f"Cannot import camera application:\n{str(e)}")
                return
            ...
            self.load_button.show()
            QMessageBox.critical(self, "Loading Error", f"Failed to load camera application:\n{str(e)}")
...
class MainWindow(QMainWindow):
    ...
    def _collapse_sidebar(self): self._animate_to(self._collapsed_width)
    ...
    def _build(self):
        ...
        self.splitter.setStyleSheet("QSplitter::handle { background:#3a3f44; } QSplitter::handle:hover { background:#4b5156; }")
        root.addWidget(self.splitter, 1)

        # Sidebar
        self.side = QFrame(); self.side.setObjectName("Sidebar")
        self.side.setMinimumWidth(120); self.side.setMaximumWidth(320)
        self.side.setStyleSheet("QFrame#Sidebar{background-color:#2b2f33; border-right:1px solid #3a3f44;}")
        self.side.setMouseTracking(True); self.side.installEventFilter(self)
        sv = QVBoxLayout(self.side); sv.setContentsMargins(0,0,0,0); sv.setSpacing(0)

        header = QFrame(); header.setFixedHeight(110)
        hv = QHBoxLayout(header); hv.setContentsMargins(12,8,12,8); hv.setSpacing(0)
        logo_label = AspectLogoLabel(); logo_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        logo_file = _find_logo()
        if logo_file is not None:
            pm = QPixmap(str(logo_file))
            if not pm.isNull(): logo_label.setPixmap(pm)
            else: logo_label.setStyleSheet("background:#2b2f33; border-radius:8px;")
        else:
            logo_label.setStyleSheet("background:#2b2f33; border-radius:8px;")
        hv.addWidget(logo_label); sv.addWidget(header)

        nav = QFrame(); nv = QVBoxLayout(nav); nv.setContentsMargins(4,18,4,18); nv.setSpacing(6)
        self._buttons_cfg = [
        ...
        for text, idx, tip in self._buttons_cfg:
            b = ToolButton(text, tooltip=tip); b.clicked.connect(lambda _, i=idx: self.switch_tool(i)); nv.addWidget(b)

        nv.addSpacing(10)
        line = QFrame(); line.setFrameShape(QFrame.HLine); line.setFrameShadow(QFrame.Sunken); line.setStyleSheet("color:#3a3f44;")
        nv.addWidget(line)
        section = QLabel("MEASUREMENTS")
        section.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        section.setStyleSheet("color:#94a3b8; font-size:11px; font-weight:800; margin:10px 12px 2px 12px; letter-spacing:1px;")
        nv.addWidget(section)
        b_roi = ToolButton("  ROI ", tooltip="Region of Interest & Measurements")
        b_roi.clicked.connect(lambda _: self.switch_tool(4)); nv.addWidget(b_roi)

        nv.addStretch(1); sv.addWidget(nav, 1)

        # Main area
        self.main_frame = QFrame(); self.main_frame.setStyleSheet("QFrame{background:#1f2327;}")
        mv = QVBoxLayout(self.main_frame); mv.setContentsMargins(0,0,0,0); mv.setSpacing(0)

        self.pathway = PathwayIndicator(); mv.addWidget(self.pathway)
        ...
        self.splitter.addWidget(self.side); self.splitter.addWidget(self.main_frame)
    ...
    def _restore_window_session(self):
        try:
            ...
            if not geo.isEmpty(): self.restoreGeometry(geo)
            st = self.prefs.get_win_state()
            if not st.isEmpty(): self.restoreState(st)
            if self.prefs.get_maximized(False): self.showMaximized()
        except Exception:
            pass
